...
const steps = [
  ...
  { label: 'Generating skill gap report', icon: '📊' },
  { label: 'Writing AI suggestions', icon: '✨' },
]
...
const LoadingScreen = ({ darkMode }) => {
  ...
  useEffect(() => {
    const stepTimer = setInterval(() => setCurrentStep(prev => Math.min(prev + 1, steps.length - 1)), 1800)
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-8">
      ...
      <div className="text-center">
        <p className="text-lg font-display font-semibold">{steps[currentStep].label}{dots}</p>
        <p className={`text-sm mt-1 ${darkMode ? 'text-white/30' : 'text-gray-400'}`}>This usually takes 10–15 seconds</p>
      </div>
      <div className="flex gap-2">
        {steps.map((_, i) => (
          <div key={i} className="h-1.5 rounded-full transition-all duration-500"
            style={{ width: i === currentStep ? '24px' : '8px', background: i <= currentStep ? '#14b268' : 'rgba(255,255,255,0.1)' }} />
        ))}
      </div>
    </div>
  )
